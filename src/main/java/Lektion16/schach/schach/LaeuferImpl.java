...
public class LaeuferImpl extends AbstractFigur implements Laeufer
{
  ...
  public Brett gibErlaubteFelder()
  {
    ...
    for (int i = 1; i <= 8; i++)
    {
      if(x+i <=8&&y+i<=8)brett.markiereFeld(x+i, y+i);
      if(x-i >=1&&y+i<=8)brett.markiereFeld(x-i, y+i);
      if(x+i <=8&&y+i>=1)brett.markiereFeld(x+i, y-i);
      if(x-i >=1&&y+i>=1)brett.markiereFeld(x-i, y-i);
    }
    return brett;
  }
